...
- **[`<model-viewer>`](https://modelviewer.dev/) 3.0.1** — renders the AR models; provides optional native AR handoff (iOS Quick Look) via its own AR button.
...
## The route
Defined in `route.js`. Current stops: REI Washington DC → Union Market → La Cosecha → Eunia → Red Bear Brewing → The Rigby. Each stop has coordinates, a geofence radius, a 3D model, and clue text.

`ROUTE_VERSION` in `app.js` (`union-market-6stop-v1`) is bumped whenever stops are added/removed/reordered so stale saved progress is discarded and rebuilt from `route.js`.
...
GitHub Pages caches `app.js`, `styles.css`, and `route.js` for 10 minutes (`max-age=600`). They're loaded with `?v=N` query params in `index.html` — **bump `N` on every change to those files** or phones will run stale code. Current: `app.js?v=11`, `styles.css?v=9`, `route.js?v=6`.
