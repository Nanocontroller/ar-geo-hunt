...
6. Tap **Close & continue**.
   - Expected: overlay closes, progress becomes `1 / 6`, the map flies to the next checkpoint, and the status pill now reflects checkpoint 2 (Union Market).

7. For checkpoints 2–6 (no dedicated buttons for these), use either:
   ...
   | Checkpoint | Lat | Lng |
   |---|---|---|
   | Union Market | 38.908306 | -76.997250 |
   | La Cosecha | 38.908778 | -76.999444 |
   | Eunia | 38.907966 | -77.001971 |
   | Red Bear Brewing | 38.905570 | -77.002481 |
   | The Rigby | 38.906031 | -77.002184 |

   Each one should auto-pop its AR overlay the same way step 5 did.

8. On checkpoint 6's (The Rigby) AR overlay, also tap **model-viewer's own built-in AR icon** (small icon in the corner of the 3D view — not the app's "Close & continue" button, and not the same thing as the automatic camera feed in step 5). This is optional/secondary: it hands off to real iOS Quick Look for true world-anchored placement, using the same camera. Worth checking since it's unrelated to GPS and is the one piece of native AR behavior that only real iOS hardware can validate.

9. After closing checkpoint 6's AR, confirm the victory overlay appears.
...
3. Use the location simulation controls to load a custom GPX file containing waypoints at the 6 checkpoint coordinates above, with a walking pace.
